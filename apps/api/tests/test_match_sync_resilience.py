...
def _no_cache_invalidation():
    with patch("app.service.match_sync.invalidate_dashboards_for_participants", AsyncMock()) as invalidate:
        yield invalidate
...
class TestReEnrich:
    async def test_stops_after_consecutive_riot_failures(self):
        ...
        with patch("app.service.match_sync.riot_client", return_value=client), patch(
            ...
            updated = await re_enrich_missing_quest_stats(_db(), _player(), rows, limit=3)

        assert updated == 0
        assert client.fetch_participant.await_count == 2

    async def test_copies_recomputed_quest_fields_and_invalidates_dashboards(self, _no_cache_invalidation):
        row = _s26_row("EUW1_1")
        ...
        _no_cache_invalidation.assert_awaited_once()
...
class TestBackfillRoleBoundItems:
    async def test_reads_reward_item_from_match_payload(self):
        ...
        with patch("app.service.match_sync.riot_client", return_value=client), patch(
            ...
            updated = await backfill_role_bound_items(_db(), _player(), [row])
...
class TestSyncFallback:
    async def test_returns_stored_matches_when_riot_sync_fails(self):
        ...
        with patch(
            ...
            AsyncMock(side_effect=[stored, stored]),
        ), patch(
            ...
            result = await sync_player_recent_matches(_db(), _player(), user_id=1, limit=20)
