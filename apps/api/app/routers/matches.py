...
from app.crud.snapshot import get_matches_by_snapshot, get_snapshot_by_id
...
def list_matches(
    ...
    return _to_responses(get_matches_by_snapshot(db, snapshot_id, user_id=current_user.id))
...
def get_stats(
    ...
    return SnapshotStatsResponse.from_matches(snapshot_id, _to_responses(matches))
...
async def get_most_played_champions(
    ...
    for m in get_history_from_cache(db, player_id):
        champion_counts[m.champion] += 1
        if m.win:
            champion_wins[m.champion] += 1

    current_version = ddragon_client.version or FALLBACK_VERSION
    return [
        MostPlayedChampionResponse(
            champion_name=name,
            games_played=count,
            win_rate=round((champion_wins[name] / count) * 100, 2),
