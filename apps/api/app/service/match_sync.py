...
async def get_stored_player_matches(
    db: AsyncSession,
    player_id: int,
    user_id: int,
    *,
    ...
    return await get_recent_matches_for_player(
...
async def backfill_role_bound_items(
    ...
    *,
    ...
    if updated:
        await db.commit()
        await invalidate_dashboards_for_participants(db, (m.id for m in updated))
        logger.info("[%s] Backfilled role_bound_item for %d matches.", player.game_name, len(updated))
    return len(updated)
...
async def re_enrich_missing_quest_stats(
    ...
    *,
    ...
    candidates = [
        ...
        and (m.quest_completion_time is None or (m.role_bound_item or 0) == 0)
    ][:limit]
    if not candidates:
        return 0
    ...
    async with create_secure_session() as session:
        for match in candidates:
            try:
                ...
                logger.warning("[%s] Quest re-enrich failed %s: %s", player.game_name, match.match_id, exc)
                continue
            if stats is None:
                consecutive_failures += 1
                logger.warning("[%s] Quest re-enrich skipped %s", player.game_name, match.match_id)
                if consecutive_failures >= _MAX_CONSECUTIVE_RIOT_FAILURES:
                    logger.warning(
                        "[%s] Stopping quest re-enrich (Riot unavailable / rate limit).", player.game_name
                    )
                    break
                continue
            consecutive_failures = 0
    ...
    if updated:
        await db.commit()
        await invalidate_dashboards_for_participants(db, (m.id for m in updated))
        logger.info("[%s] Re-enriched quest data for %d matches.", player.game_name, len(updated))
    return len(updated)
...
async def sync_player_recent_matches(
    ...
    latest = await get_latest_snapshot_for_player(db, player.id, user_id)
    participants = []
    ...
    try:
        ...
        async with create_secure_session() as session:
            if latest is None:
                logger.info("[%s] No snapshot — fetching up to %d live matches.", player.game_name, limit)
                ...
                known_ids = await get_stored_match_ids_for_player(db, player.id, user_id)
        ...
        logger.warning("[%s] Riot sync failed (%s); using stored matches.", player.game_name, exc)
        participants = []

    if participants and latest is not None:
        await persist_participants_to_snapshot(db, latest, participants)
        logger.info("[%s] Added %d matches to snapshot %d.", player.game_name, len(participants), latest.id)

    stored = await get_stored_player_matches(db, player.id, user_id, limit=limit, role_filter=role_filter)
    if stored:
        try:
            await backfill_role_bound_items(db, player, stored, limit=limit)
        except Exception as exc:
            logger.warning("[%s] role_bound_item backfill aborted (%s).", player.game_name, exc)
    if stored and needs_quest_reenrich(stored):
        try:
            await re_enrich_missing_quest_stats(db, player, stored, limit=limit)
        except Exception as exc:
            logger.warning("[%s] Quest re-enrich aborted (%s); returning cache.", player.game_name, exc)

    return await get_stored_player_matches(db, player.id, user_id, limit=limit, role_filter=role_filter)
